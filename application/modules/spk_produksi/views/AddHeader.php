				...
				<div class="row">
					...
					<div class="col-sm-12">
						...
						<div class="col-sm-6">
							<div class="form-group row">
								...
								<div class="col-md-8">
									<input type="date" class="form-control" id="tgl_penawaran" value="<?= $tanggal ?>" onkeyup required name="tgl_penawaran" value='<?=$tgl_spk_produksi;?>'>
								</div>
							</div>
						</div>
					</div>
					...
					<div class="col-sm-12">
						...
						<div class="col-sm-6">
							<div class="form-group row">
								<div class="col-md-4">
									<label for="no_penawaran">Count m</label>
								</div>
								<div class="col-md-4">
					...
					<div class="col-sm-12">
						...
						<div class="col-sm-6">
							<div class="form-group row">
								...
								<div class="col-md-8">
								  	<input type='hidden' class='form-control autoNumeric' id='berat_terpakai' name='berat_terpakai' readonly value='<?=$JUMLAH;?>'>
								    <input type='text' class='form-control autoNumeric' id='tipe' name='tipe' readonly>
								</div>
							</div>
						</div>
								...
								<table class='table table-bordered table-striped'>
									...
									foreach ($results['detail'] as $dt_spk){$loop++; 
										...
										echo"</select></th>
											...
											<th id='width3_$loop'><input type='text' class='form-control'   		value='$dt_spk->width_akhir' 			readonly id='used_weight3_$loop' required name='dt[$loop][weight3]'></th>
											<th hidden id='qtycoil_$loop'><input type='text' class='form-control'   		value='$dt_spk->qty_coil' 			readonly id='qtycoil_$loop' required name='dt[$loop][qtycoil]'></th>
									...
									<tfoot>
									    ...
									    <tr>
										 ...
										 <tr>
											...
											<th width='15%'></th>
											<th width='15%'></th>
											<th width='10%'>Scrap trimming + Scrap sisa</th>
											<th width='7%'></th>
											<th width='7%'></th>
											<th width='7%'>
											</th>
											<th width='7%'>
											<input type='text' class='form-control' id='scrapsisa' onkeyup required name='scrapsisa' readonly >
											</th>
											<th width='7%'>											
											</th>											
											<th width='7%'></th>
										 ...
										 <tr>
											...
											<th width='7%'></th>
											<th width='7%'>
											<input type='hidden' class='form-control' id='efisiensipanjang' onkeyup required name='efisiensipanjang' readonly >
											</th>
											<th width='7%'>
...
<script type="text/javascript">
	...
	function Caristock(){
		...
		$.ajax({
            type:"GET",
            url:siteurl+'spk_produksi/caritipe',
	...
	function CariDetail(id){
		...
		$.ajax({
            type:"GET",
            url:siteurl+'spk_produksi/partNumber',
            data:"id_marketing="+id_marketing+"&id="+id,
            success:function(html){
               $('#part_number_'+id).html(html);
            }
        });
		$.ajax({
            type:"GET",
		...
		$.ajax({
            type:"GET",
            url:siteurl+'spk_produksi/CariW3material',
            data:"id_marketing="+id_marketing+"&id="+id+"&width="+width+"&kg_process="+kg_process,
            success:function(html){
               $('#width3_'+id).html(html);
				
            }
        });
		$.ajax({
            type:"GET",
            url:siteurl+'spk_produksi/CariW4material',
            data:"id_marketing="+id_marketing+"&id="+id+"&width="+width+"&kg_process="+kg_process,
            success:function(html){
               $('#qtycoil_'+id).html(html);
				
            }
        });
		$.ajax({
            type:"GET",
            url:siteurl+'spk_produksi/CariW5material',
            data:"id_marketing="+id_marketing+"&id="+id+"&width="+width+"&kg_process="+kg_process,
            success:function(html){
               $('#estimasi_'+id).html(html);
	...
	const changeChecked = () => {
		...
		let scrap   = dataX.kg_process - SUM_KG;
		let jumlah_pisau = SUM_COIL * 2 + 2;
		let sisa_potongan = dataX.width - dataX.lpegangan - SUM_PANJANG;//SUM_TOT;
		let sisa_weight   = dataX.weight - SUM_KG - scrap;
		
		let efisiensiberat   = SUM_KG/dataX.kg_process * 100;
		let efisiensipanjang   = sisa_potongan/dataX.width * 100;
		
		console.log(scrap);
		//console.log(dataX.kg_process);
		
		$('#scrapsisa').val(number_format(scrap,2));
	...
	function cariIdcustomer(id){
		var customer = $('#used_namacustomer_'+id).val();
		$('#used_idcustomer_'+id).val(customer);
	}
