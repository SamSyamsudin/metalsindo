<?php
	...
	$id_trspkproduksi = (!empty($header))?$header[0]->id_tr_spk_produksi:'';
	...
	$no_urut  = (!empty($header))?$header[0]->no_urut:'';
		...
		<form id="data-form" method="post">
			<div class="col-sm-12">
				<div class="row">
								...
								<div class="col-md-8" id='lotnumbe_slot'>
									<select id="id_stock" name="id_stock" class="form-control select" onchange="get_produk()" required>
										...
										<?php foreach ($results['stock'] as $val){
											...
											?>
											<?php echo"<option value='$val->id_stock' $selx >$val->lotno|$val->width|$val->length|$val->totalweight kg</option>"; ?>
						...
						<div class="col-sm-6">
							...
							<div class="form-group row">
								...
								<div class="col-md-8" id="slot_width">
									...
									<input type='hidden' class='form-control' id='id_spkproduksi'  required name='id_spkproduksi' readonly value='<?=$id_trspkproduksi;?>' >
									
									<input type='hidden' class='form-control' id='id_spkproduksi2'  required name='id_spkproduksi2' readonly value='<?=$id_spkproduksi;?>' >
									
									<input type='hidden' class='form-control' id='no_urut'  required name='no_urut' readonly value='<?=$no_urut;?>' >
								...
								<table class='table table-bordered table-striped'>
									...
									foreach ($results['detail'] as $dt_spk){$loop++; 
										...
										echo"</select></th>
											...
											<th id='keterangan_$loop'><input type='text' class='form-control input-sm' value='$dt_spk->keterangan' id='used_keterangan_$loop' required name='dt[$loop][keterangan]'></th>
											";
					...
					<center>
						<?php if(empty($results['view'])){ ?>
						<button type="submit" class="btn btn-success btn-sm" name="save" id="simpan-com"><i class="fa fa-save"></i>Update</button>
						<?php } ?>
			...
			swal({
				...
				function(isConfirm) {
				  if (isConfirm) {
						...
						var baseurl=siteurl+'spk_produksi/SaveNewHeader_edit';
						$.ajax({
							url			: baseurl,
							type		: "POST",
							data		: formData,
