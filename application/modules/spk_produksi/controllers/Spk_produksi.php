...
class Spk_produksi extends Admin_Controller
{
	...
	public function index2()
    {
       $this->auth->restrict($this->viewPermission);
        ...
        $data2 = $this->Inventory_4_model->CariSPK3($id);
		$selTab = (!empty($_SESSION['JSON_Filter']))? $_SESSION['JSON_Filter']['tabnya']:'';
        $this->template->set('results', $data);
        $this->template->set('results2', $data2);
        $this->template->set('selTab', $selTab);
	...
	public function addHeader_new($id=null,$view=null){
		...
		$stock		= $this->db->query("SELECT a.* FROM stock_material a WHERE a.totalweight > 0")->result();
		$karyawan 	= $this->Inventory_4_model->get_data('ms_karyawan','deleted',$deleted);
		$material 	= $this->Inventory_4_model->get_data_category3();
		$mata_uang 	= $this->Inventory_4_model->get_data('mata_uang','deleted',$deleted);
		...
		$stok	= [];
		if(!empty($header)){
			$stok	= $this->db->get_where('stock_material',array('id_category3'=>$header[0]->id_material,'id_gudang'=>'1','totalweight !='=>0))->result_array();
		}
				...
				foreach($stok as $stok){
					...
					if(!empty($detail_potonganx)){
						...
						if($stok->weight > $JUMLAH){
							echo"<option value='$stok->id_stock'>$stok->lotno|$stok->width|$stok->length|$stok->totalweight kg</option>";
						}
					}
					else{
						echo"<option value='$stok->id_stock'>$stok->lotno|$stok->width|$stok->length|$stok->totalweight kg</option>";
		...
		<tr id='tr_$loop'>
			...
			<td>
				<select id='used_no_surat_$loop' name='dt[$loop][no_surat]' data-no='$loop' onchange='CariDetail($loop)' class='form-control select' required>
					...
					echo"<option value='$nosurat->id_dt_spkmarketing'>$nosurat->no_surat|$nosurat->width</option>";
					}
		echo	"</select>
			</td>
			...
			<th hidden  id='width3_$loop'><input type='text' class='form-control'   		value='$dt_spk->width_akhir' 			readonly id='used_weight3_$loop' required name='dt[$loop][weight3]'></th>
			...
			<th hidden id='qtycoil1_$loop'><input type='text' class='form-control'   		value='$dt_spk->qty_coil' 			readonly id='qty_coil1_$loop' required name='dt[$loop][qty_coil1]'></th>
			<th hidden id='estimasi_$loop'><input type='text' class='form-control'   		value='$dt_spk->estimasi' 			readonly id='used_estimasi_$loop' required name='dt[$loop][estimasi]'></th>
			...
			<td id='part_number_$loop' hidden><input type='text' class='form-control'  		value='$dt_spk->part_number' 	readonly id='used_part_number_$loop' required name='dt[$loop][part_number]'></td>
			...
			<td id='keterangan_$loop'><input type='text' class='form-control input-sm' id='used_keterangan_$loop' required name='dt[$loop][keterangan]'></td>
...
function CariKeterangan()
    {
		...
		$dbstocking	= $this->db->query("SELECT * FROM dt_spkmarketing WHERE id_dt_spkmarketing = '$id_marketing' ")->result();
		$readonly = ($id_marketing == 'nonspk')?'':'readonly';
		$id_customerr = $dbstocking[0]->keterangan;
		echo "<input type='text' class='form-control input-sm'  value='$id_customerr' id='used_keterangan_$loop' required name='dt[$loop][keterangan]'>";
...
function partNumber()
    {
		...
		$dbstocking	= $this->db->query("SELECT * FROM dt_spkmarketing WHERE id_dt_spkmarketing = '$id_marketing' ")->result();
		$readonly = ($id_marketing == 'nonspk')?'':'readonly';
		$id_customerr = $dbstocking[0]->part_number;
		echo "<input type='text' class='form-control input-sm'  value='$id_customerr' id='used_part_number_$loop' required name='dt[$loop][part_number]'>";
...
function CariW2material()
	...
	function CariW3material()
    {
		...
		$dbstocking		= $this->db->query("SELECT * FROM dt_spkmarketing WHERE id_dt_spkmarketing = '$id_marketing' ")->result();
		$width_row 		= $dbstocking[0]->width_akhir;
		echo "<input type='text' class='form-control input-sm' value='".number_format($width_row,2)."' id='used_weight3_$loop' required name='dt[$loop][weight3]' readonly>";
	}
	function CariW4material()
    {
        $loop			= $_GET['id'];
		$width			= (!empty($_GET['width']))?$_GET['width']:0;
		$kg_process		= $_GET['kg_process'];
		...
		$dbstocking		= $this->db->query("SELECT * FROM dt_spkmarketing WHERE id_dt_spkmarketing = '$id_marketing' ")->result();
		$width_row 		= $dbstocking[0]->qty_coil;
		echo "<input type='text' class='form-control input-sm' value='".number_format($width_row,2)."' id='used_qtycoil_$loop' required name='dt[$loop][qtycoil]' readonly>";
	}
	function CariW5material()
    {
        $loop			= $_GET['id'];
		$width			= (!empty($_GET['width']))?$_GET['width']:0;
		$kg_process		= $_GET['kg_process'];
		...
		$dbstocking		= $this->db->query("SELECT * FROM dt_spkmarketing WHERE id_dt_spkmarketing = '$id_marketing' ")->result();
		$width_row 		= $dbstocking[0]->estimasi;
		echo "<input type='text' class='form-control input-sm' value='".number_format($width_row,2)."' id='used_estimasi_$loop' required name='dt[$loop][estimasi]' readonly>";
	}
...
function CariNamaCustomer()
    {
		...
		echo" <select id='used_namacustomer_$loop' name='dt[$loop][namacustomer]' onchange='cariIdcustomer($loop)' class='form-control select'>
			 ";
...
function cari_inven1()
	...
	public function Approve(){
		...
		if($jumlahstock <= '1'){
			...
			$this->db->where('id_spkproduksi',$id)->update("tr_spk_produksi",$data);
			$this->db->where('id_spkproduksi',$id)->update("dt_tr_spk_produksi",$data);
	...
	public function Approve_new(){
		...
		if($jumlahstock <= '1'){
			...
			$this->db->where('id_tr_spk_produksi',$id)->update("tr_spk_produksi",$data);
	...
	public function SaveNewHeader()
    {
		...
		$data = [
							...
							'kg_process'			=> str_replace(',','',$post['totalweigthproses']),
							...
							'no_urut'				=> 1,
							'planning_proses_width'	    => str_replace(',','',$post['totalpanjangslitting2']),
							'planning_proses_weight'	=> str_replace(',','',$post['kg_process']),
							'efisiensi_width'			=> str_replace(',','',$post['efisiensipanjang']),
							'efisiensi_weight'			=> str_replace(',','',$post['efisiensiberat']),
							'sisa_kg'					=> str_replace(',','',$post['sisaweight']),
							'scrap_sisa'				=> str_replace(',','',$post['scrapsisa']),
							'tahun'					    => date('Y-m-d'),
							'type'					    => $tipe,
                            ];
		...
		foreach($_POST['dt'] as $used){
			if(!empty($used[no_surat])){
				...
				if($used[no_surat]=='nonspk'){
					$idcustomer  = $used[idcustomer];
					$cust        = $this->db->query("SELECT name_customer FROM master_customers WHERE id_customer='$idcustomer'")->row();
					$customer    = $cust->name_customer;
				}else{
					$customer =$used[namacustomer];
				}
				
				$dt[] =  array(
						'id_spkproduksi'		=> $code,
						'no_surat'				=> $used[no_surat],
						'id_dt_spkproduksi'		=> $code.'-1'.'-'.$numb1,
						'idcustomer'			=> $used[idcustomer],
						...
						'checked'				=> $checked,
						'id_tr_spk_produksi'    => $code.'-1',
						'part_number'			=> $used[part_number],
						'qty_coil'			    => $used[qty_coil],
						'estimasi'			    => $used[estimasi]
						);
			}
		}
		...
		$JUMLAH += str_replace(',','',$post['totalweigthproses']);
		
		$ArrNew[] = [
			'kode' => $code,
			'id_stock' => $post['id_stock'],
			'berat' => str_replace(',','',$post['totalweigthproses']),
			'id_gudang' => 1
		];
		...
		$this->db->trans_start();
			...
			$this->db->update('stock_material',array('detail_potongan'=>$new_Array, 'sisa_potongan'=>$JUMLAH,'on_proses'=>'Y', 'sisa_spk'=>str_replace(',','',$post['sisaweight'])));
	...
	public function SaveNewHeader_edit()
    {
		...
		$no_surat = $post['no_surat'];
		$code = $post['id_spkproduksi'];
		$code2 = $post['id_spkproduksi2'];
		$no_urut =  $post['no_urut'];
		$idstocknew = $post['id_stock'];
		
		$mat = $this->db->query("SELECT * FROM tr_spk_produksi WHERE id_spkproduksi='$code2'")->row();
		$idstock = $mat->id_stok;
		$weight  = $mat->totalweight;
		
			if($idstocknew != $idstock){
					$stock = [							
							 'aktif'			=> 'Y',
							 'detail_potongan'  => '',
							 'sisa_spk'         => $weight
					...
					$this->db->update('stock_material',$stock);
			}
		}
		...
		$this->db->trans_start();
			...
			$this->db->where('id_stock',$post['id_stock']);
			$this->db->update('stock_material',array('detail_potongan'=>$new_Array, 'sisa_potongan'=>$JUMLAH, 'sisa_spk'=>$post['sisaweight']));
	...
	public function SaveNewHeaderproses()
    {
		...
		$this->db->trans_start();
			...
			$this->db->where('id_stock',$post['id_stock']);
			$this->db->update('stock_material',array('detail_potongan'=>$new_Array, 'sisa_potongan'=>$JUMLAH, 'sisa_spk'=>str_replace(',','',$post['sisaweight'])));
	...
	function CariStokfg()
    {
		...
		$stok = $this->db->query("SELECT sum(totalweight) as totalqty FROM stock_material WHERE id_category3='$dt_spk->id_material' AND width='$dt_spk->width' AND thickness='$dt_spk->thickness' AND id_gudang='3' AND aktif='Y'")->row();
	...
	function update_stok_sisa(){
		
		
		$sql = $this->db->query("SELECT * FROM stock_material")->result();
		...
		$id_stock = $valx->id_stock;
				...
				$dbstock		= $this->db->query("SELECT * FROM stock_material WHERE id_stock = '$id_stock' ")->result();
				      ...
				      foreach ($dbstock as $valstok => $valxstok ){	
				
				
						$detail_potongan = (!empty($valxstok->detail_potongan))?json_decode($valxstok->detail_potongan, true):array();
						// print_r($detail_potongan);
						
						$JUMLAH = 0;
						if(!empty($detail_potongan)){
							foreach($detail_potongan AS $valdt => $valxdt){
									// echo 'masuk 1';
									$JUMLAH += $valxdt['berat'];
							}
						}
						...
						$weight 		= $valxstok->weight - $JUMLAH;
						
						
						$this->db->query("UPDATE stock_material 
									SET sisa_spk='$weight'
									WHERE id_stock='$valx->id_stock'");
	...
	public function addHeaderBooking($id=null,$view=null){
		$this->auth->restrict($this->viewPermission);
        $session = $this->session->userdata('app_session');
		$this->template->page_icon('fa fa-pencil');
		$aktif = 'active';
		...
		$material 	= $this->db
						->select('a.id_stock,a.weight, a.width, b.lotno, b.nama_material, b.thickness, c.nama, c.maker, d.id_spkmarketing, e.name_customer')
						->from('stock_material_customer a')
						->join('stock_material b','a.id_stock=b.id_stock')
						->join('ms_inventory_category3 c','b.id_category3=c.id_category3')
						->join('dt_spkmarketing d','a.id_dt_spkmarketing=d.id_dt_spkmarketing')
						->join('master_customers e','a.id_customer=e.id_customer')
						->where('a.sts_over',1)
						->where('d.status_lanjutan',2)
						->where('a.proses','OPN')
						->order_by('a.id','DESC')
						->get()
						->result();
        ...
        $this->template->set('results', $data);
        $this->template->title('SPK Produksi');
        $this->template->render('AddHeaderBooking');
	...
	public function SaveNewHeaderBooking()
    {
        $this->auth->restrict($this->addPermission);
		$post = $this->input->post();
		...
		$this->db->trans_start();
			...
			$this->db->where('id_stock',$idstock);
			$this->db->update('stock_material_customer',array('proses'=>'CLS'));
	...
	function caritipe()
    {
		...
		$kategory3	= $this->db->query("SELECT * FROM ms_inventory_category3 WHERE id_category3 = '$id_category3' ")->result();
		$maker = $kategory3[0]->maker;
		
		if($maker =='JASA SLITTING'){
		$tipe ='slitting';
		}else{
		$tipe ='sales';		
		}
