			...
			<table id="mytabledata" class="table table-bordered">
				...
				<tbody>
					...
					if (!empty($data)) {
						...
						foreach ($data as $record) {
							...
							<tr>
								...
								<td class="exclass"><input type="text" class="form-control tanggal" id="tanggal_<?= $numb ?>" name="tanggal_<?= $numb ?>" value="" placeholder="Tanggal"></td>
								<td class="exclass">
					...
					<tr class="exclass">
						...
						<td colspan=2><input type="text" class="form-control divide input-sm" name="total_req" id="total_req" value="0" readonly></td>
					</tr>
				</tbody>
			</table>
			<div class="pull-right"><button type="button" id="btnxls" class="btn btn-default">Export Excel</button> <button type="submit" name="save" class="btn btn-success btn-sm" id="submit"><i class="fa fa-save">&nbsp;</i>Update</button></div>
		</div>
	</div>
	<!-- /.box-body -->
</div>
...
<script type="text/javascript">
	...
	$("#btnxls").click(function() {
		$("#mytabledata").table2excel({
			...
			filename: "RequestPayment.xls", // do include extension
