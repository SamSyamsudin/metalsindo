...
class Expense extends Admin_Controller
{
	...
	{
		parent::__construct();
		$this->load->model(array('all/All_model', 'Expense/Expense_model', 'All/All_model', 'Jurnal_nomor/Jurnal_model'));
	...
	public function kasbon_save()
	{
		...
		if ($id != "") {
			...
			$result = $this->db->update(DBERP . '.tr_kasbon', $data, array('id' => $id));
			...
			$id = $this->db->insert(DBERP . '.tr_kasbon', $data);
			// if (is_numeric($id)) {
			// 	$result = TRUE;
			// } else {
			// 	$result = FALSE;
	...
	{
		$data = $this->All_model->GetOneTable('tr_kasbon', array('nama' => $nama, 'departement' => $departement, 'status' => '3'), 'tgl_doc');
		$check = $this->db->get_where('tr_kasbon', ['nama' => $nama, 'departement' => $departement, 'status' => 3])->num_rows();
		if ($check > 0) {
			$data = $this->db->get_where('tr_kasbon', ['nama' => $nama, 'departement' => $departement, 'status' => 3])->result();
		} else {
			$data = false;
		}

		// print_r($nama . ' ' . $departement);
...
*/
		...
		if ($id != "") {
			...
			if (!empty($detail_id)) {
				foreach ($detail_id as $keys => $val) {
					...
					if ($qty[$keys] > 0) {
						$config['upload_path'] = './assets/expense/';
						$config['allowed_types'] = 'gif|jpg|png|jpeg|bmp|pdf|doc|docx|xls|xlsx|jfif';
						$config['remove_spaces'] = TRUE;
						$config['encrypt_name'] = TRUE;
	...
	public function review($id)
	{
		...
		$data_user = $this->All_model->GetUsersCombo();
