...
class Delivery_order extends Admin_Controller
{
		...
		public function EditHeader()
    {
        ...
        $this->template->render('NewEditHeader');
...
function AddMaterial_old()
	...
	function GetPenawaran(){
		...
		foreach($dt AS $dt){
			...
			<tr class='baris_".$loop."'>
				...
				<td rowspan='1' class='id_".$loop."'><input type='text' class='form-control input-sm text-right' value='".number_format($dt->total_qty,2)."' name='hd[$loop][qty_produk]' id='qty_$loop' readonly></td>
				<td rowspan='1' class='id_".$loop."'>
	...
	function getStockLot(){
		...
		$lotnumber  = (!empty($get_stock[0]->lotno))?$get_stock[0]->lotno:'-';
...
function cari_inven1()
	...
	public function SaveNewHeaderEdit() {
		...
		foreach($_POST['dp'] as $val => $dt){
			$numb1++;
					...
					$ArrDetail[$val.$vaX]['id_stock']			    = $idstock->id_stock;
					$ArrDetail[$val.$vaX]['qty']			    	= str_replace(',','',$dt['qty']);
					$ArrDetail[$val.$vaX]['weight']			    	= str_replace(',','',$dt['weight']);
					$ArrDetail[$val.$vaX]['qty_mat']			    = str_replace(',','',$dt['qty_mat']);
					$ArrDetail[$val.$vaX]['weight_mat']			    = str_replace(',','',$dt['weight_mat']);
		...
		foreach($_POST['dp2'] as $val => $dt2){
			...
			$numb2 =$numb1+$no;
					...
					$ArrDetail2[$val.$vaX]['id_delivery_order']		= $id_delivery;
					$ArrDetail2[$val.$vaX]['id_dt_delivery_order']	= $id_delivery.'-'.$numb2;
					$ArrDetail2[$val.$vaX]['id_material']		    = $dt2['id_material'];
					$ArrDetail2[$val.$vaX]['nm_material']		    = $dt2['material'];
					$ArrDetail2[$val.$vaX]['no_alloy']		        = $dt2['no_alloy'];
					$ArrDetail2[$val.$vaX]['thickness']		        = $dt2['thickness'];
					...
					$ArrDetail2[$val.$vaX]['bantuan']			    = $dt2['bantuan'];
					$ArrDetail2[$val.$vaX]['id_dt_spkmarketing']	    = $dt2['id_dt_spkmarketing'];
					$ArrDetail2[$val.$vaX]['lotno']			        = $dt2['lot'];
					$ArrDetail2[$val.$vaX]['lot_slitting']			= $dt2['lot'];
	...
	public function ReleaseDO(){
		...
		foreach ($get_list as $key => $value) {
			...
			$kirim 		= $value['weight_mat'];
			$this->db->query("UPDATE stock_material SET status_do ='CLS' WHERE id_stock='$id_stock'");
			$this->db->query("UPDATE stock_material SET sisa_spk = sisa_spk - $kirim, total_kirim = $kirim, deleted='1', aktif='N', no_kirim='$id'
		...
		foreach ($temp as $key => $value) {
			...
			if(!empty($rest_pusat)){
				...
				$ArrStock[$key]['totalweight'] 	= $TWG;
				$ArrStock[$key]['weight'] 		= $SAT;
				$ArrStock[$key]['sisa_spk'] 	= $TWG;
				$ArrStock[$key]['aktif'] 		= 'N';
				$ArrStock[$key]['status_do'] 		= 'CLS';
				$ArrStock[$key]['total_kirim'] 		= 'CLS';
				$ArrStock[$key]['deleted'] 		= 1;
			}
		}
...
public function deletePenawaran(){
	...
	public function PrintHeaderWord(){
	...
	public function PrintHeaderHtml(){
		
        $this->auth->restrict($this->managePermission);
        $id = $this->uri->segment(3);
		$data['header'] = $this->Delivery_order_model->getHeaderPenawaran($id); 
		$data['detail']  = $this->Delivery_order_model->PrintDetail($id);
		$this->load->view('PrintHeaderHtml',$data);
	}
	
	public function PrintHeaderHtml2(){
		
        $this->auth->restrict($this->managePermission);
        $id = $this->uri->segment(3);
		$data['header'] = $this->Delivery_order_model->getHeaderPenawaran($id); 
		$data['detail']  = $this->Delivery_order_model->PrintDetail($id);
		$this->load->view('PrintHeader2HTML',$data);
	}
	
	public function update_stok_spk(){
			$id_stock 	= $this->input->post('idstok');	
			$this->db->query("UPDATE stock_material SET no_surat ='BATAL DO', customer=null, booking=0, delivery_book=0 WHERE id_stock='$id_stock'");
			
	}
	
	public function tes_nomor(){	  
	  $no_surat = $this->Delivery_order_model->BuatNomorNew();
      print_r($no_surat);
	...
	public function addHeaderSlitting()
    {
		...
		$karyawan = $this->Delivery_order_model->get_data('ms_karyawan','deleted',$deleted);
		$mata_uang = $this->Delivery_order_model->get_data('mata_uang','deleted',$deleted);
		$produksi = $this->Delivery_order_model->get_produksi();
		$customer = $this->db
							->select('a.id_customer, b.name_customer')
							->from('tr_spk_marketing a')
							->join('master_customers b','a.id_customer=b.id_customer','left')
		...
		$id_material = $this->Delivery_order_model->get_data('ms_inventory_category3','deleted',$deleted);		
		$id_gudang = $this->Delivery_order_model->get_data('ms_gudang');		
        ...
        $this->template->set('results', $data);
        $this->template->title('Add Delivery Order');
        $this->template->render('AddHeaderSlitting');
    }
	...
	public function TambahData(){
		...
		$mt     = $this->db->query("SELECT * FROM stock_material WHERE keterangan = '".$no_po."'  AND id_gudang='4' ")->result();
		foreach($mt as $material){
			
			$id_material  = $material->id_category3;
		...
		<td>
			<input  type='hidden' 		value='".$material->id_stock."'	class='form-control input-sm' id='id_stock_".$id."_".$no."' 		required name='dt[".$id."][detail][".$no."][id_stock]' 	readonly>
			<input  type='text' 		value='".$id_material."'	class='form-control input-sm' id='id_material_".$id."_".$no."' 		required name='dt[".$id."][detail][".$no."][id_material]' 	readonly>		
		</td>
		<td><input  type='text' 		value='".$material->nama_material."'	class='form-control input-sm' id='nama_material_".$id."_".$no."' 		required name='dt[".$id."][detail][".$no."][nama_material]' 	readonly></td>
		<td><input  type='text' 		value='".$material->lotno."'	class='form-control input-sm' id='lotno_".$id."_".$no."' 		required name='dt[".$id."][detail][".$no."][lotno]' 	readonly></td>
		<td><input  type='text' 		value='".$material->qty."'	class='form-control input-sm' id='qty_".$id."_".$no."' 		required name='dt[".$id."][detail][".$no."][qty]' 	readonly></td>
		<td><input  type='text' 		value='".$material->sisa_spk."'	class='form-control input-sm' id='berat_".$id."_".$no."' 		required name='dt[".$id."][detail][".$no."][berat]' 	readonly></td>
		<td><input  type='text' 		class='form-control input-sm' id='remark_".$id."_".$no."' 		required name='dt[".$id."][detail][".$no."][remark]'></td>
	...
	public function SaveNewHeaderSlitting(){
        $this->auth->restrict($this->addPermission);
		$post 		= $this->input->post();
    ...
    foreach($Detail AS $val => $valx){
      ...
      foreach($valx['detail'] AS $val2 => $valx2){
		  ...
		  $thick = $this->db->query("SELECT thickness FROM ms_inventory_category3 WHERE id_category3='$idmaterial'")->row();
		  
		  $numb3 = $numb1+$numb1+$no1;
			        $ArrDetail3[$val2.$val]['id_delivery_order']		= $code;
					$ArrDetail3[$val2.$val]['id_dt_delivery_order']	= $code.'-'.$numb3;
					$ArrDetail3[$val2.$val]['id_material']		    = $valx2['id_material'];
					$ArrDetail3[$val2.$val]['nm_material']		    = $valx2['nama_material'];
					$ArrDetail3[$val2.$val]['no_alloy']		        = $valx2['no_alloy'];
					$ArrDetail3[$val2.$val]['thickness']		        = $thick->thickness;
					$ArrDetail3[$val2.$val]['width']		        	= str_replace(',','',$valx2['width']);
					$ArrDetail3[$val2.$val]['length']		        	= str_replace(',','',$valx2['length']);
					$ArrDetail3[$val2.$val]['qty_order']			    = str_replace(',','',$valx2['qty_produk']);

					$ArrDetail3[$val2.$val]['id_stock']			    = $valx2['id_stock'];
					$ArrDetail3[$val2.$val]['qty']			    	= str_replace(',','',$valx2['qty']);
					$ArrDetail3[$val2.$val]['weight']			    	= str_replace(',','',$valx2['berat']);
					$ArrDetail3[$val2.$val]['qty_mat']			    = str_replace(',','',$valx2['qty_mat']);
					$ArrDetail3[$val2.$val]['weight_mat']			    = str_replace(',','',$valx2['berat']);
					$ArrDetail3[$val2.$val]['remark']			    	= $valx2['remarks'];
					$ArrDetail3[$val2.$val]['bantuan']			    = $valx2['bantuan'];
					$ArrDetail3[$val2.$val]['id_dt_spkmarketing']	    = $valx2['id_dt_spkmarketing'];
					$ArrDetail3[$val2.$val]['lotno']			        = $valx2['lotno'];
					$ArrDetail3[$val2.$val]['lot_slitting']			= $valx2['lot'];
					

					$ArrDetail3[$val2.$val]['created_by']			    = $this->auth->user_id();
					$ArrDetail3[$val2.$val]['created_on']			    = date('Y-m-d H:i:s');
		...
		$this->db->trans_start();
			...
			if(!empty($Detail)){
			$this->db->insert_batch('dt_delivery_order_child_scrap',$ArrDetail3);
			}
	...
	public function PrintHeaderWordSlitting(){
