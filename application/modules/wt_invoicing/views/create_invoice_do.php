		    ...
		    <div class="col-sm-6">
		        <div class="form-group row">
			        ...
			        <div class="col-md-8" hidden>
				        ...
				        <input type="text" class="form-control" id="tipe" value="reguler" required name="tipe" readonly placeholder="tipe">
					</div>
			        <div class="col-md-8">
					...
					<input type="text" class="form-control" id="note" required name="note" value="<?= $term->payment_term ?>" placeholder="Terms">
						...
						<table id="example1" class="table table-bordered table-striped">
									...
									foreach ($results['detail'] as $dt_spk){
										...
										$harga    = $this->db->query("SELECT a.* FROM view_dodt_spkmarketing as a WHERE a.no_spk_marketing='$dt_spk->no_spk_marketing' AND a.id_material='$dt_spk->id_material' AND a.width='$dt_spk->width'")->row();
                                        ...
                                        <tr id='tr_$loop'>
											...
											<td id='qty_so_$loop'><input type='text' class='form-control input-sm' id='used_qty_so_$loop' required name='dt[$loop][qty_so]' onblur='HitungTotal($loop)' value='$dt_spk->total_kirim' readonly></td>
											<td id='qty_$loop' hidden><input type='text' class='form-control input-sm' id='used_qty_$loop' required name='dt[$loop][qty]' onblur='HitungTotal($loop)' value='$dt_spk->total_kirim' readonly></td>
