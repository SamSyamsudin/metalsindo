...
class Budget_rutin_model extends BF_Model
{
	...
	function GetBudgetRutin(){
		$this->db->select('a.*, b.nama AS nm_dept, c.cost_center');
		$this->db->from('budget_rutin_header a');
		$this->db->join('ms_department b','a.department=b.id', 'left');
	...
	function GetBudgetRutinDetail($code_budget){
		$this->db->select('a.*, b.stock_name nama_barang, b.spec spec1, c.nm_category nama_jenis, c.id AS id_type, z.code AS nm_satuan');
        ...
        $this->db->join('ms_satuan z','a.satuan=z.id','left');
