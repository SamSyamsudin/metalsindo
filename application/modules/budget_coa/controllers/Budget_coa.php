			...
			for ($x = 0; $x < count($id); $x++) {
				if ($jenis[$x] == 'FIX COST BULANAN') {
					$data =  array(
						...
						'bulan_9' => $bulan_9[$x], 'bulan_10' => $bulan_10[$x], 'bulan_11' => $bulan_11[$x], 'bulan_12' => $bulan_12[$x], 'total' => $total[$x],
						'created_by_dept' => $this->auth->user_id(),
						'created_on_dept' => date('Y-m-d H:i:s')
					);
				} else {
					$data =  array(
						'jenis' => $jenis[$x], 'nilai' => $nilai[$x], 'variabel_coa' => $variabel_coa[$x], 'status' => '2',
						'created_by_dept' => $this->auth->user_id(),
						'created_on_dept' => date('Y-m-d H:i:s')
					);
				}
